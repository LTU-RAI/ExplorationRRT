...
/// Port
/// Can be overriden by the user
const BIND_PORT_DEFAULT: u32 = 3300;
...
fn execution_handler(
    ...
    match initial_guess {
        ...
        Some(u0) => {
            if u0.len() != RRT_NUM_DECISION_VARIABLES {
    ...
    if parameter.len() != RRT_NUM_PARAMETERS {
...
fn main() {
    let matches = App::new("OpEn TCP Server [rrt]")
        .version("0.0.0")
        .author("John Smith")
